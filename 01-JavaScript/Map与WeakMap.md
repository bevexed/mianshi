...
| 迭代         | 需要先取 keys                                 | 本身可迭代                               |
| 原型与特殊键 | 普通对象继承原型；需留意 `__proto__` 等键     | 不走对象属性和原型链                     |
| 频繁增删     | 语义不如 Map 直接                             | API 专为键值集合设计；实际性能需基准测试 |
...
- **WeakMap 的 key 必须是对象或非全局注册的 Symbol**，且不会阻止 key 被回收——
...
对象没有其他强引用后，对应缓存条目可被自动释放，能降低长期缓存泄漏风险

**3. 记录 DOM 节点的关联数据**——节点从文档移除后自动清理

### 面试加分点

普通 `Map` 会强引用对象 key；如果长生命周期 Map 持续加入 key 且不删除，
这些对象不会被回收。Map 本身不是泄漏，缺少容量或清理策略才是问题。
