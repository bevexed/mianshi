...
- `optional`——短暂等待，超出交换期后该次导航通常继续使用回退字体，
  能降低换字体带来的 CLS，但要接受自定义字体可能不展示
...
Layout Instability API 会用 `hadRecentInput` 排除近期离散用户输入
（通常为 500ms 窗口）直接导致的位移。
例如用户点击展开手风琴，紧随的内容下移通常不计入 CLS。

- 交互产生的后续异步位移如果超出近期输入窗口，仍可能计入 CLS。
- 持续改变 layout 的长动画不仅可能计入 CLS，也可能造成渲染压力；
  优先使用 transform 类动画，并尊重 `prefers-reduced-motion`。
