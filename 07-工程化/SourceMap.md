...
| `false`                        | 不生成                           | 不允许产生 map 且接受线上栈难还原的特殊场景 |

---

## 生产环境的正确做法

1. 生成不向产物附加公开 URL 的 map
   （Webpack 可用 `hidden-source-map`，Vite 可用对应的 hidden sourcemap 选项）
2. **上传到 Sentry 后从服务器删除**，不部署到 CDN
3. 这样错误能还原源码，但**用户拿不到 map 文件**

### 为什么通常不公开部署 map

map 可能包含原始文件路径、未压缩源码和注释，会显著降低阅读门槛。
客户端代码本身不应存放秘密，但 sourcemap 仍应按组织的知识产权、
隐私和调试需求决定是否公开。常见做法是仅上传到受控错误监控服务。
